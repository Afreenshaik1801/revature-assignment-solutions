...
CREATE TABLE Room (
  ...
  CONSTRAINT fk_Room_Block_PK FOREIGN KEY(BlockFloor, BlockCode)
...
CREATE TABLE On_Call (
  ...
  CONSTRAINT fk_OnCall_Block_Floor FOREIGN KEY(BlockFloor, BlockCode)
    REFERENCES Block(BlockFloor, BlockCode)
);
...
-- 1.The names of all physicians that have performed a medical procedure they have never been certified to perform.


select p.Name from Physician as p
join undergoes as u 
on p.EmployeeID = u.Physician
where u.procedures not in (select treatment from trained_in t where p.EmployeeID = t.Physician);
...
select p.Name as Physician_Name, po.name as Procedure_Name, u.dateundergoes as Date_Undergone,pt.Name as Patient_Name
from Physician as p
join undergoes as u on p.EmployeeID = u.Physician
join procedures as po on po.code = u.Procedures
...
(select treatment from trained_in t where p.EmployeeID = t.Physician and u.DateUndergoes > t.CertificationExpires);


-- 4. me as the previous query, but include the following information in the results: 
...
select p.Name, po.name, u.DateUndergoes, pt.name from Physician as p
join undergoes as u  on p.EmployeeID = u.Physician
join procedures as po on u.procedures = po.Code
join patient as pt on pt.ssn = u.patient
where u.procedures in 
(select treatment from trained_in t where p.EmployeeID = t.Physician and u.DateUndergoes > t.CertificationExpires);
...
select p.name, psp.name,n.name,a.starto,a.endo,a.examinationroom,ps.name from appointment as a
join patient as p on p.SSN = a.patient
join physician ps on ps.employeeid = p.pcp
join physician psp on psp.employeeid = a.physician
left join nurse n on n.employeeid = a.prepnurse
where p.pcp <> a.physician;



-- 6. The Patient field in Undergoes is redundant, since we can obtain it from the Stay table. There are no constraints in force to prevent inconsistencies between these two tables. 
-- More specifically, the Undergoes table may include a row where the patient ID does not match the one we would obtain from the Stay table through the Undergoes.Stay foreign key. Select all 
-- rows from Undergoes that exhibit this inconsistency


select u.* from undergoes u
left join stay s on s.stayid = u.stay
where s.patient <> u.patient or s.stayid is null or s.patient is null or u.patient is null;

-- 7. Obtain the names of all the nurses who have ever been on call for room 123.

select n.name from nurse n
join on_call cl on cl.nurse = n.employeeid
join room r on cl.blockfloor = r.blockfloor and cl.blockcode = cl.blockcode
...
select examinationroom,count(*) as appointments from appointment
...
select pa.Name as PatientName, pc.Name as PCPName
from Patient pa join Physician pc on pa.PCP = pc.EmployeeID
where exists (
    -- prescribed by the patient's PCP
    select 1 from Prescribes pr
    where pr.Patient = pa.SSN and pr.Physician = pa.PCP
)
and exists (
    -- underwent a procedure costing > 5000
    select 1 from Undergoes u
    join Procedures pr2 on u.Procedures = pr2.Code 
    where u.Patient = pa.SSN and pr2.Cost > 5000
)
and (
    -- at least two appointments with a registered prep nurse
    select COUNT(*)
    from Appointment a
