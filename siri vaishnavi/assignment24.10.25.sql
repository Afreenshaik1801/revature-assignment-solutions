...
-- 1
SELECT 
Name FROM Physician AS T1
INNER JOIN Undergoes AS T2
ON T1.EmployeeID = T2.Physician
WHERE T2.Procedures NOT IN (SELECT Treatment FROM Trained_In WHERE Physician = T1.EmployeeID);
-- 2
SELECT
  T1.Name AS Physician_Name,
  T3.Name AS Procedure_Name,
  T2.DateUndergoes AS Procedure_Date,
  T4.Name AS Patient_Name
FROM Physician AS T1
INNER JOIN Undergoes AS T2
  ON T1.EmployeeID = T2.Physician
INNER JOIN Procedures AS T3
  ON T2.Procedures = T3.Code
INNER JOIN Patient AS T4
  ON T2.Patient = T4.SSN
WHERE
  T2.Procedures NOT IN (
    SELECT
      Treatment
    FROM Trained_In
    WHERE
      Physician = T1.EmployeeID);
...
SELECT pt.Name AS PatientName,
       ...
       n.Name  AS PrepNurseName,         
       a.Starto AS StartTime,
       a.Endo  AS EndTime,
       a.ExaminationRoom AS ExaminationRoom,
...
LEFT JOIN Physician pcp
  ON pt.PCP = pcp.EmployeeID
WHERE a.Physician != pt.PCP;
...
JOIN Room r
  ON r.BlockFloor = oc.BlockFloor
 AND r.BlockCode  = oc.BlockCode
WHERE r.RoomNumber = 123;

-- 8
SELECT ExaminationRoom, COUNT(*) AS AppointmentCount
FROM Appointment
...
SELECT pt.Name  AS PatientName,
       pcp.Name           AS PCPName
FROM Patient pt
JOIN Physician pcp ON pt.PCP = pcp.EmployeeID
WHERE 
  -- 1) prescribed by their PCP
  EXISTS (
    SELECT 1
    FROM Prescribes pr
  ...
  -- 2) undergone a procedure costing > 5000
  AND EXISTS (
    SELECT 1
    FROM Undergoes u
  ...
  -- 3) at least two appointments prepped by a registered nurse
  AND (
    SELECT COUNT(*)
    FROM Appointment a
  ...
  -- 4) PCP not head of any department
  AND NOT EXISTS (
    SELECT 1
    FROM Department d
    WHERE d.Head = pt.PCP);
